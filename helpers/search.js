...
var search = function (search_term, cb) {
    ...
    if (terms.length > 0 && terms[0].charAt(0) === '@') {
      ...
      sql = "SELECT DISTINCT concat('<a href=https://ancient-plateau-84686.herokuapp.com/movie/', m.movie_id,'>')  ref,  title, release_year, runtime, rating FROM Movie m JOIN (Cast_In ci JOIN (SELECT id FROM Movie_Cast WHERE name = '" + new_search_term + "') c ON c.id = ci.cast_id) ON ci.movie_id = m.movie_id LIMIT 20;";
    } else {
      var bi_conditions = '';
      var uni_conditions = '';
      var prev_term = '';
      ...
      let sql1 = "SELECT DISTINCT concat('<a href=https://ancient-plateau-84686.herokuapp.com/movie/', movie_id,'>')  ref,  title, release_year, runtime, rating FROM Movie WHERE title LIKE '%" + search_term + "%'";
      // Second query will be searching for the bigram parts of the search
      let sql2 = "SELECT DISTINCT concat('<a href=https://ancient-plateau-84686.herokuapp.com/movie/', movie_id,'>')  ref,  title, release_year, runtime, rating FROM Movie " + bi_conditions;
      // Third query will be searching for the individual parts of the search
      let sql3 = "SELECT DISTINCT concat('<a href=https://ancient-plateau-84686.herokuapp.com/movie/', movie_id,'>')  ref,  title, release_year, runtime, rating FROM Movie " + uni_conditions;
